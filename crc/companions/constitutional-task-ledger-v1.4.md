...
### v1.4 Conceptual Architecture Paper, Companion 8 to Constitutional Runtime Computation v5.14
...
- oar_id: OAR-F003
  ...
  effect: Task Ledger Bind outcome references consume the Core v5.14 BindingRecord reconciliation while preserving Task Ledger ownership of CycleRecord and PendingObligation.

Source OAR Entry:
- oar_id: OAR-F003
  status: SPECIFYING
  role: governing source finding for BindingRecord consumer alignment
...
These closure targets are not dependencies declared under the Open Architecture Dependencies field.
...
Seven candidate objects were considered. Two are folded or rejected on inspection, an editorial decision recorded here rather than left silent. `TaskState` is folded into `TaskLedger.status`, reusing Constitutional Boundary Contracts' own `GoalStatusEnum` rather than building a second status vocabulary the corpus would then need to reconcile. That enum carries the value RECONSTITUTED, the status of a task under active reconstitution. Under the R4 reconciliation, Boundary Contracts owns the seven-value `GoalStatusEnum` including RECONSTITUTED; this paper reuses the enum and supplies the operational meaning and mechanics of reconstitution rather than owning the enum value itself. RECONSTITUTED is used explicitly in III.1 below. `TaskProgressRecord`, named in the original working material, is rejected as redundant: a chronological progress picture is already fully reconstructable by querying a TaskLedger's own `cycle_history` alongside its `PendingObligation` population, and a third object serving the same function would duplicate rather than add structure.
...
A thin, first-class index entry, deliberately not a duplicate of content already pinned elsewhere. Carries: `cycle_id`, `task_id`, `prior_resolution_ref`, `outcome_type` (`Bind | Hold | Escalate | NonFormation | StandingFailure`), `outcome_ref` (a pointer to the actual typed record CTLC, Standing, Boundary Contracts, or Core BindingRecord already produced for this cycle), `obligations_created` (PendingObligation references), `obligations_resolved` (PendingObligation references resolved this cycle), `no_obligation_reason` (populated only when a triggering event under Part VI's table would ordinarily be expected to create an obligation and did not, naming why, closing the silent-drop path Part VI's own hardened rule now forbids), `affordance_set_delta_ref` (a pointer into AllowedNextAffordanceSet's own event log, present only if this cycle changed the affordance set), and `created_at`.

For `CycleRecord.outcome_type = Bind`, `outcome_ref = BindingRecordRef`. BindingRecord is Core-owned. CycleRecord consumes it. CycleRecord does not create binding and does not own BindingRecord. BindingRecord does not point back to CycleRecord.

If a valid governed mutation bound but BindingRecord persistence failed or is uncertain, ordinary CycleRecord Bind must not point to a missing object. Binding may already have occurred, and the missing record does not erase mutation, but ordinary closure waits for BindingRecord confirmation or reconstruction. After reconstruction, `outcome_ref` points to the valid BindingRecord.

For `source_event_type = BindingClosureRepairRequired`, `source_event_ref` must identify one or more of GovernedStateMutationRecord, attempted BindingRecord identity, durable write-failure evidence, audit event, CycleRecord write failure, or Task Ledger write failure. `no_obligation_reason` is prohibited. Obligation creation is mandatory. Ordinary continuation is blocked. Ordinary cycle closure is blocked when BindingRecord is missing. Restricted repair or escalation continuation may issue only if separately authorized. The obligation remains open until repair completes.

A binding-closure repair obligation closes only when BindingRecord is confirmed or reconstructed, CycleRecord Bind is written where required, Task Ledger update succeeds, durable audit lineage exists, duplicate or conflict checks pass, and obligation resolution is recorded.

If CycleRecord write fails after bind, BindingRecord remains valid and the bind remains historically valid. Task Ledger must carry a repair obligation, and continuation remains restricted until cycle closure is repaired.

If Task Ledger update fails after bind, bind and CycleRecord may remain historically valid, but the ledger repair obligation must be durably visible. The next ordinary continuation must not silently bypass the failed update.
...
**F-5 alignment note.** Generalized actuation result states and admission states are likewise not CycleRecord outcome_type values. ActuationResult, PENDING_ADMISSION, ADMITTED, PARTIALLY_ADMITTED, REJECTED, ADMISSION_HELD, ADMISSION_ESCALATED, VERIFICATION_REQUIRED, result unknown, partial effect, compensation, and audit-repair conditions are represented through PendingObligation, no_obligation_reason, or the typed F-5 objects that the CycleRecord outcome_ref may eventually index through F-3. This paper preserves the existing CycleRecord outcome_type vocabulary: Bind, Hold, Escalate, NonFormation, and StandingFailure.
...
## IV.4 Generalized F-5 obligation accounting

Constitutional Runtime Computation v5.13 and Constitutional Actuation and Effect Admission v1.0 distinguish actuation result status, admission status, governed mutation, and binding. This paper owns the task-level obligation consequence of those distinctions, not the generic F-5 objects themselves.

ActuationResult and admission outcomes are not automatically CycleRecord outcome types. A failed, rejected, unknown, partial, held, escalated, or verification-required admission state must produce either a PendingObligation or an explicit no_obligation_reason. The same rule applies when compensation is required or fails, and when audit replication is delayed under a ratified degraded-mode rule. CycleRecord retains the terminal constitutional cycle outcome. Task Ledger update follows BindingRecord or another lawful terminal non-bind outcome. ContinuationState may issue only after obligations are accounted for. An unresolved or unknown actuation result may not disappear through cycle closure.

F-3 remains responsible for the final Bind outcome_ref target. Future BindingRecord integration must conform to the F-5 dependency contract, but this v1.3 alignment does not settle the final BindingRecord schema.
...
*v1.4: F-3 BindingRecord reconciliation. Aligns Constitutional Task Ledger with Core v5.14. Settles CycleRecord Bind outcome lineage by requiring `CycleRecord.outcome_type = Bind` to use `outcome_ref = BindingRecordRef`. Preserves the CycleRecord outcome vocabulary and does not add F-5 result statuses. Adds `BindingClosureRepairRequired` as a PendingObligation source and adds obligation_type values `binding_record_persistence_failed`, `binding_record_persistence_uncertain`, `cycle_record_write_failed_after_bind`, and `task_ledger_update_failed_after_bind`. Requires mandatory binding-closure repair obligations when a governed mutation has bound but BindingRecord, CycleRecord, or Task Ledger persistence failed or is uncertain. Prohibits no_obligation_reason for those cases, blocks ordinary continuation, and preserves Task Ledger ownership without owning BindingRecord. No em dashes.*

*v1.3: F-5 Phase 1 alignment. Aligns Constitutional Task Ledger with Constitutional Runtime Computation v5.13 and Constitutional Actuation and Effect Admission v1.0. Preserves CycleRecord.outcome_type as Bind, Hold, Escalate, NonFormation, and StandingFailure. Clarifies that ActuationResult and admission states are not CycleRecord outcome types. Expands PendingObligation.source_event_type to cover AdmissionHeld, AdmissionEscalated, VerificationRequired, ResultUnknown, PartialEffect, ExternalEffectNotAdmitted, CompensationRequired, CompensationFailed, and AuditReplicationPending while preserving Hold, Escalate, NonFormation, StandingFailure, and ToolResultHold. Adds a generalized F-5 obligation-accounting rule requiring failed, rejected, unknown, partial, held, escalated, verification-required, compensation, and delayed-audit cases to produce either a PendingObligation or an explicit no_obligation_reason. Defers final Bind outcome_ref settlement to F-3 BindingRecord reconciliation. No em dashes.*

*v1.2: R4 reconciliation applied on the reuse side. RECONSTITUTED is realigned from this paper's own extension of GoalStatusEnum to a value Constitutional Boundary Contracts owns under Core Appendix B. Part III now states that Boundary Contracts owns the seven-value GoalStatusEnum including RECONSTITUTED, and that this paper reuses the enum and supplies the operational meaning and mechanics of reconstitution rather than owning the enum value itself. RECONSTITUTED projects to the Core coarse goal_status IN_PROGRESS, declared in Core Appendix B, B.6. The reading-order citation of Constitutional Boundary Contracts is updated to v1.2. Aligns with Constitutional Runtime Computation v5.5. No change to the Constitutional Task Ledger Principle, the seven governed objects, either predicate, the obligation-carrying table, the P_task family, or the reconstitution apparatus, ReconstitutionRecord and reconstitution_history. No em dashes.*
