...
### v1.1 Conceptual Architecture Paper, Companion 9 to Constitutional Runtime Computation v5.5
...
## IV.0 F-4 Reconciliation and Object Ownership

Constitutional Feedback v1.0's claim to extend Memory's `LearningCandidate` and `FeedbackObservation` was non-operative under Constitutional Runtime Computation Appendix B B.2. The claim stated a divergent definition of Memory-owned shared apparatus, but no B.3 amendment declaration and no B.3.1 owner back-reference joined the definitions. Constitutional Feedback v1.1 withdraws that extension claim.

Constitutional Memory v2.2 remains the canonical owner of the Hold-specific `LearningCandidate` and `FeedbackObservation`. This paper does not amend, rename, narrow, redefine, extend, or replace those objects. Existing Memory consumers remain valid against Memory v2.2.

This paper instead defines two C9-owned operational generalizations under distinct names: `OperationalLearningCandidate` and `OperationalFeedbackObservation`. They inherit the constitutional pattern of substrate-mediated, source-typed, scope-bounded learning, but not the identity or ownership of Memory's Hold-specific objects. The mapping in IV.2-A is an interoperability mapping only. It does not confer shared identity, transfer ownership, or create amendment authority over Memory's schema.

## IV.1 OperationalLearningCandidate

An **OperationalLearningCandidate** is a C9-owned typed proposal to convert a governed operational outcome into a scoped future-facing lesson, affordance, memory operation, or review path. It is scoped to the operational branch only. Doctrinal feedback does not produce an OperationalLearningCandidate; it produces a ReconstitutionFeedbackRecord (IV.3).

An OperationalLearningCandidate carries: `operational_learning_candidate_id`, `source_class` (always `OperationalOutcome` on this object), `source_ref` (a typed union over the operational source records governed by Part V: HoldRecord, EscalationState, NonFormationReceipt, StandingFailureRecord, ToolResultRecord, or ObligationPatternRecord), `source_transition_ref` where the source has one, `failed_conjunct` where applicable, `failure_class` where applicable, `proposed_lesson`, `lesson_scope`, `lesson_type`, `target_memory_tier`, `target_principal`, `base_memory_operation_ref`, `authority_requirement`, `overgeneralization_risk`, `bypass_risk`, `future_affordance_effect`, `expiry_or_review_condition`, `source_context_refs`, `task_ref`, `status` (`PROPOSED | HELD | ADMITTED | ESCALATED`), and `audit_ref`.

`source_ref` is the generalized operational source pointer this paper needs for cross-surface learning. It is not Memory's `source_hold_ref` renamed in place. For a Hold-derived operational candidate, `source_ref` may point to the source HoldRecord, but the C9 object remains distinct from Memory's LearningCandidate. `source_class` never takes `DoctrinalReconstitution` on this object; doctrinal propagation is routed through ReconstitutionFeedbackRecord and DoctrinalFeedbackReachable(δ).

## IV.2 OperationalFeedbackObservation

An **OperationalFeedbackObservation** is the C9-owned operational observation object produced when an admitted operational lesson has an observation-shaping effect. It generalizes the operational observation pattern across Hold, Escalate, NonFormation, StandingFailure, ToolResultHold, and PendingObligation resolution patterns. It is not Memory's FeedbackObservation and does not replace the Memory object.

An OperationalFeedbackObservation carries: `operational_feedback_observation_id`, `source_class` (always `OperationalOutcome` on this object), `source_ref`, `operational_learning_candidate_ref`, `task_ref`, `continuation_state_ref`, `issued_in_cycle`, `feedback_summary`, `failed_conjunct` where applicable, `correction_scope`, `allowed_next_affordances`, `prohibited_replay_notice` where applicable, `required_state_change_for_resubmission` where applicable, `valid_escalation_routes`, `memory_view_refs`, `feedback_effect_type`, `effect_ref`, `expiry`, `issued_by`, `created_at`, and `audit_ref`.

Doctrinal propagation does not produce an OperationalFeedbackObservation. A propagated correction is a change to a standard, not an agent-facing operational lesson about how to act differently next cycle, and its typed output is ReconstitutionFeedbackRecord.

## IV.2-A Memory-to-Operational Mapping

A Hold-specific Memory `LearningCandidate` can be represented in the C9 operational population by creating or referencing a distinct OperationalLearningCandidate. Memory `LearningCandidate.source_hold_ref` maps to C9 `OperationalLearningCandidate.source_ref`, with `source_class` fixed to `OperationalOutcome`. Memory's candidate identifier is preserved as predecessor provenance or mapped into the C9 operational candidate identifier, but that preservation does not prove the two objects are identical. Memory's lesson, scope, tier, target principal, base memory operation, authority, risk, context, expiry or review condition, and audit information are preserved in the corresponding C9 fields.

A Hold-specific Memory `FeedbackObservation` can be represented in the C9 operational population by creating or referencing a distinct OperationalFeedbackObservation. Memory `FeedbackObservation.prior_hold_ref` maps to C9 `OperationalFeedbackObservation.source_ref`. Memory `FeedbackObservation.learning_candidate_ref` maps to `operational_learning_candidate_ref`. Memory's continuation, cycle, correction, affordance, replay, state-change, escalation, memory-view, expiry, issuer, timestamp, and audit information remain preserved where applicable.

This mapping allows Memory's Hold-specific objects to participate in Feedback's operational population without changing the Memory objects themselves. It does not create shared identity, ownership transfer, a Memory amendment, or a B.3.1 back-reference obligation.

## IV.3 ReconstitutionFeedbackRecord (new), with an explicit lifecycle

The operational branch's own lifecycle is explicit by construction: a source outcome is linked through PendingObligation, evaluated under `OperationalFeedbackSourceValid(σ)`, proposed as an OperationalLearningCandidate, evaluated under `FeedbackLearningReachable(λ)`, and, if admitted, produces the applicable typed effect and a FeedbackLedgerEntry. Where the admitted effect is observation shaping, it produces an OperationalFeedbackObservation. The doctrinal branch needs the same explicitness rather than leaving ReconstitutionFeedbackRecord to serve, ambiguously, as both its own proposal and its own final record. Its lifecycle is: a source reconstitution is confirmed on its own surface, a candidate target surface is named, a ReconstitutionFeedbackRecord is opened in `PROPOSED` status, `DoctrinalFeedbackReachable(δ)` is evaluated against it, and the record is finalized to `CONFIRMED`, `HELD`, or `ESCALATED`, at which point a FeedbackLedgerEntry is written referencing it.
...
FeedbackLedgerEntry:
  ...
  source_surface
  target_surface
  operational_learning_candidate_ref
...
An entry's `source_class` determines which of `operational_learning_candidate_ref` or `reconstitution_feedback_record_ref` is populated. `no_feedback_reason` is populated only when a feedback-eligible outcome, per Part VII's tables, was assessed and deliberately produced no lesson or propagation, closing the silent-drop path exactly as Task Ledger's own `no_obligation_reason` closes it for obligations. **The governing rule:** a feedback-eligible outcome must either be indexed in the FeedbackLedger, via a populated `operational_learning_candidate_ref` or `reconstitution_feedback_record_ref`, or receive a recorded `no_feedback_reason`. Silence is never a legitimate outcome of this table.
...
Verdict structure. Identical in shape to Memory’s own but using C9-owned operational objects: an OperationalLearningCandidate that satisfies FeedbackLearningReachable(λ) is finalized to ADMITTED and produces the applicable typed feedback effect. Where that effect is observation shaping, the substrate issues an OperationalFeedbackObservation. A failed conjunct finalizes the candidate to HELD, logged with the specific failing conjunct. An undecidable or sovereign-routed case finalizes it to ESCALATED. A memory-tier target still passes through the applicable Memory operation authority rather than by identity with Memory’s Hold-specific candidate.
...
flowchart TB
    ...
    LC["OperationalLearningCandidate\n(+ OperationalFeedbackObservation if\nobservation-shaped)"]
...
*Undetectable at a single lesson.* Yes. Each individual OperationalLearningCandidate admission and each individual ReconstitutionFeedbackRecord confirmation is independently scoped, independently typed, and independently defensible against Part V's or Part VI's own conjuncts.
...
| This paper | OperationalLearningCandidate, ReconstitutionFeedbackRecord | Yes (both branches) | Yes (two predicates) | Yes (P_fbk4) | Feedback-authority as reachability |
...
**OperationalLearningCandidate.** C9's operational object for a source-typed, scope-bounded lesson candidate. It carries source_class, always OperationalOutcome, and source_ref, a typed union over the operational source records reached through PendingObligation. It is distinct from Memory's Hold-specific LearningCandidate.

**OperationalFeedbackObservation.** C9's operational observation object for admitted operational lessons with an observation-shaping effect. It carries source_class, source_ref, operational_learning_candidate_ref, task_ref, continuation, cycle, correction, affordance, replay, state-change, escalation, memory-view, expiry, issuer, timestamp, and audit information. It is distinct from Memory's Hold-specific FeedbackObservation.
