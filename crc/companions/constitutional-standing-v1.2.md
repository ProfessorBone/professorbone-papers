...
### v1.2 Conceptual Architecture Paper, Companion 6 to Constitutional Runtime Computation v5.4; substantial advancement of the task-state continuity residue named in v5.4's Open Problems, and formalization of the Standing (STAND) step named but not specified in CTLC's CRA Assembly
...
- `failed_check`: `ProposalWellFormed | BoundaryConformant | ContinuationGrounded`, naming which part of Formation failed.
...
- `boundary_contract_ref`: the specific boundary contract version BoundaryConformant was evaluated against.
...
- `authority_context_ref`: the StandingContext (Part III) in force at the moment of the attempt, since a formation failure can still involve a specific presenting standing instance even when Formation itself is what fails.
...
**StandingContext** is the typed object StandingValid(σ) is evaluated against, and it is not a new invention so much as a name finally given to something §6a already silently presupposed: the HoldRecord's `authority_context_ref` field pins a reference to "the authority context A, the requester's standing and the authority graph, at evaluation time," without §6a itself defining what that object is. This paper supplies the definition. A StandingContext carries: `standing_id` (the specific granted instance, distinct from the standing class it instantiates), `standing_class_ref` (which class, for example a pre-authorized delegated class), `granted_by` (the authorizing sovereign act), `granted_at` and `valid_until` (the validity window), `task_ref` and `cycle_ref` (the scope within which this instance is current), and `revoked` (null unless and until withdrawn, with a `revoked_by` and `revoked_at` when set). `authority_context_ref`, wherever it appears in this paper, §6a, or any future companion, resolves to a StandingContext.
...
```
CRC parent (v5.4):          Reachable(τ), CRA Assembly (STAND named, one sentence),
...
**Recommended reading order:** Constitutional Runtime Computation v5.4; Constitutional Boundary Contracts v1.0; Constitutional Memory v2.1; Constitutional Retrieval v1.2; Constitutional Baselines v1.2; Constitutional Coherence v1.2; Constitutional Thresholds v1.2; Constitutional Standing v1.2 (this paper).
...
**StandingContext.** The typed object StandingValid is evaluated against, and the object §6a's authority_context_ref already presupposed without defining: standing_id, standing_class_ref, granted_by, granted_at, valid_until, task_ref, cycle_ref, revoked.
...
**ContinuationState.** The substrate-issued object that authorizes the next ORSR cycle, generalizing AgentObservation to name explicitly what it must and must not carry: no element of the agent's own private reasoning from the prior cycle survives into it.
