...
AgentObservation is owned by Constitutional Runtime Computation Core. Core v5.11 defines its canonical constitutional schema. This paper owns the Substrate-to-Agent crossing contract, ObservationContract, and the validation predicate, ObservationConformant. ObservationContract does not own, redefine, reduce, or replace AgentObservation. It validates the crossing representation of the Core-owned object.
...
ObservationContract validates two authority categories: the Core AgentObservation fields present in the crossing representation, and the Boundary Contracts-owned metadata needed to validate that crossing. These categories do not create a separate named envelope object or a second runtime record.

### V.2.1 Core AgentObservation fields validated at the crossing

The following fields are defined canonically in Constitutional Runtime Computation v5.11 and validated here.
...
### V.2.2 ObservationContract crossing metadata

| Field | Type | Constitutional function |
|---|---|---|
| `contract_version` | ContractVersionRef | Pins the active ObservationContract version; required for audit reconstruction |

`contract_version` is owned by Boundary Contracts. It belongs to the crossing representation and is not part of the Core-owned AgentObservation semantics. Carrying this metadata does not create a separate named envelope object and does not create a second runtime record.
...
**SchemaComplete(o):** all required Core AgentObservation fields and all required ObservationContract crossing metadata are present and non-null. Conditional Core fields are present when their conditions are met.
...
ObservationContract requires the substrate to validate the crossing representation of its own output before issuing it. The validation applies the canonical Core AgentObservation field schema together with Boundary Contracts-owned crossing metadata. This is necessary, but it creates a structural problem: if the component that generates an observation also validates it, the generator can self-certify. A generator that drifts in what it produces can drift in what it accepts. The validation is inside the same trust boundary as the generation. This is not a minor implementation concern. It is the same problem the parent paper identifies at the Act layer: the governing function cannot certify its own fidelity.
...
| `contract_version` | The contract version applied in this validation; audit metadata, not a canonical AgentObservation field |
...
*v1.3: F-2 AgentObservation reconciliation. Core ownership of AgentObservation is formalized in Constitutional Runtime Computation v5.11, where the canonical AgentObservation schema is published. This paper clarifies that ObservationContract validates the crossing representation of the Core-owned object, and that ObservationConformant remains this paper's validation predicate. `contract_version` is retained as Boundary Contracts-owned crossing metadata, not as a canonical AgentObservation field. No new named envelope object and no second runtime record are introduced. No consumer migration is required. CRC-SSR v1.3 records the F-2 closure and settles AgentObservation to REGISTERED. This is the validation-view clarification accompanying F-2 reconciliation, not a B.3.1 amendment back-reference. Aligns with Constitutional Runtime Computation v5.5 under Option Y.*
