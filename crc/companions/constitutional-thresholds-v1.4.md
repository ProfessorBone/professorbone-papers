...
### v1.4 Conceptual Architecture Paper, Companion 5 to Constitutional Runtime Computation v5.5; closure of the threshold-authority and evidence-packet-provenance residues named in Constitutional Baselines v1.2 and Constitutional Coherence v1.4
...
`ThresholdPinsAdmissible(γ)` is the sole conjunct this paper adds to Coherence's GenerationPinAdmissible. It requires that every threshold-bearing member's ThresholdPinAdmissible (Part III) holds, and is vacuously true for a generation with no threshold-bearing members, so the extension imposes no obligation on a generation that has not opted into threshold governance. Per-member baseline validity is not re-checked on the routine pin; it remains where Coherence places it, in the coherence gate's MembersValid, so this paper's routine-pin extension adds the threshold check alone and leaves Coherence's four-conjunct routine pin otherwise unchanged.
...
**Amendment declarations under Core Appendix B.** The extensions above are governed compatible extensions of objects Constitutional Coherence owns, declared here under Core Appendix B, Section B.3. GenerationCoherenceReachable, amended from Constitutional Coherence v1.3, is extended by the added conjunct ThresholdRelationsCoherent, which supersedes no existing element and is vacuously true for a generation with no threshold-bearing members, so existing consumers remain valid without migration. GenerationPinAdmissible, amended from Constitutional Coherence v1.4, is extended by the single added conjunct ThresholdPinsAdmissible, which supersedes no existing element and is vacuously true for a generation with no threshold-bearing members, so existing consumers remain valid without migration. GenerationMember, amended from Constitutional Coherence v1.3, is extended by the optional field threshold_set_version, present only for threshold-governed surfaces, which supersedes no existing element and leaves existing members valid; its canonical typing is deferred to the future shared schema home rather than settled here. Constitutional Coherence carries the back-references these declarations require, and the canonical definitions remain Constitutional Coherence's until the shared schema home absorbs them. An earlier draft of this paper also listed BaselinePinsAdmissible on GenerationPinAdmissible and presented it as a rename of a Coherence pin conjunct; it has been removed under the R1 and F27 correction, because per-member baseline validity is Coherence's own MembersValid on the coherence gate, not a routine-pin conjunct, so no rename and no new routine-pin obligation is claimed.
...
**Recommended reading order:** Constitutional Runtime Computation v5.5; Constitutional Boundary Contracts v1.0; Constitutional Memory v2.1; Constitutional Retrieval v1.2; Constitutional Baselines v1.2; Constitutional Coherence v1.4; Constitutional Thresholds v1.4 (this paper).
...
**GenerationMember, extended.** The per-surface member of a BaselineGeneration, now `surface_id, baseline_version, threshold_set_version (optional)`. GenerationPinAdmissible gains ThresholdPinsAdmissible; GenerationCoherenceReachable gains ThresholdRelationsCoherent, which validates that each threshold-bearing member descends from a prior valid ThresholdChangeReachable event rather than re-adjudicating the change live on every generation use. Both added conjuncts are vacuously true for a generation with no threshold-bearing members, so no existing generation is retroactively obligated.
...
This work was developed under the Professor Bone Lab research identity as the fifth companion to Constitutional Runtime Computation v5.5, closing the threshold-authority residue named in Constitutional Baselines v1.2 and the evidence-packet-provenance residue named across Constitutional Baselines v1.2 and elevated to a deployment precondition in Constitutional Coherence v1.4. AEGIS serves as the worked domain. The v1.0 draft was written to pre-empt the recurring critiques of the prior companion review cycles: to govern the threshold and the trigger as objects distinct from the baseline rather than re-describing baseline authority with a substituted noun; to name the axis of threshold capture precisely rather than ranking it against its predecessors; to extend Companion 0's boundary-contract pattern rather than inventing a parallel evidentiary apparatus; to keep reachability separated from executability; to preserve primitive-versus-topology discipline; to reuse the B0 through B5 severity vocabulary rather than minting a parallel one; and to scope honestly rather than claiming premature closure. The v1.1 tightening pass was shaped by external critical review of v1.0 (verdict: accept with targeted revision before corpus freeze), which required the BaselineGeneration integration to be carried through formally rather than argued informally, the ThresholdSet schema expanded with a metric interface, P_thr1 separated cleanly from the trajectory-level P_thr5 rather than blurred across one worked example, P_thr3's ordinary classification narrowed to its genuinely mechanical form with a companion, explicitly open primitive for evidentiary sufficiency and candidate-selection capture, an IndependenceSufficient conjunct added to PacketConformant, a DirectSovereignAlert object defined to close the bypass-path question a B4 escalation otherwise leaves implicit, and the paper's closure language toned down from claiming the corpus's last residue to the two residues it actually closes. The v1.2 tightening pass was shaped by a second review (verdict: accept, minor revisions before corpus freeze), which confirmed the v1.1 architecture as sound and asked for five consistency and precision corrections: a stale figure label, an overstated abstract claim about the cross-surface threshold seam, a clarification that ThresholdRelationsCoherent validates lineage rather than re-adjudicating live on every generation use, an explicit acceptance statement for P_thr3b as a named primitive with its candidate-selection subtype held provisional, and a check that every closure claim in the paper is scoped to the two residues named rather than read as closing the corpus.

---

*v1.4. Cluster 6 R1 and F27 correction under Core Appendix B, Route C. BaselinePinsAdmissible is removed from the GenerationPinAdmissible routine-pin extension: it was a net-new routine-pin conjunct presented as a rename of a Coherence pin check that does not exist, and per-member baseline validity is Coherence's own MembersValid on the coherence gate, not a routine-pin conjunct. The phantom-rename provenance sentence and the Key Terms line that had GenerationPinAdmissible gain BaselinePinsAdmissible are corrected, closing F27. ThresholdPinsAdmissible is preserved as the sole added routine-pin conjunct and declared a compatible extension under Appendix B B.5 (R1a), so GenerationPinAdmissible's extension is now a clean five-conjunct form. R1 is split: R1a reconciled, R1b corrected as a phantom rename. Coherence's four-conjunct GenerationPinAdmissible is unchanged; no Core amendment. Current-owner citations of Constitutional Coherence, in reading order, acknowledgments, back-references, and body prose, are updated to v1.4; the amended-from provenance fields preserve the version each extension was amended against, so the R2 and R6 declarations remain amended from Coherence v1.3 and R1a is amended from Coherence v1.4. No change to any other predicate, the P_thr family, or the existing R2 and R6 declarations. No em dashes.*

*v1.3. Cluster 6 reconciliation, R2 and R6, under Core Appendix B. Adds formal compatible-extension amendment declarations for two objects Constitutional Coherence owns and this paper extends: GenerationCoherenceReachable, extended by ThresholdRelationsCoherent (R2), and GenerationMember, typed here with an optional threshold_set_version (R6, with canonical typing deferred to the future shared schema home). Both extensions are vacuous or optional for a generation with no threshold-bearing members, so no existing consumer is affected. GenerationPinAdmissible (R1) is deferred to a later R1 and F27 correctness pass and is not touched here. Mutual citations of Constitutional Coherence are updated to v1.3, matching its co-reconciliation in this pass. No change to any predicate, conjunct count, object, or the P_thr family. No em dashes.*
