...
### v1.7 Conceptual Architecture Paper, Companion 6 to Constitutional Runtime Computation v5.14; F-3 BindingRecord standing reconciliation
...
**Part IV** Binding: Standing contribution to BindingRecord and OperationalClosure(χ)
...
# Part IV. Binding: Standing Contribution to BindingRecord

## Why Binding is distinct from Reachability and from the verdict itself

A favorable CTLC verdict, Emit, or a processed SovereignResolution or StandingClassResolution, is not yet the same thing as substrate state actually having moved. After F-3 reconciliation, Core v5.14 owns the canonical BindingRecord schema, the generic binding point, and BindingRecordValid. This paper owns the standing contribution to that chain: StandingValid, StandingContext, standing applicability, standing non-applicability, standing validation evidence, StandingFailureRecord, and the timing rules that determine whether standing evidence may be consumed by a Core-owned BindingRecord.

Standing does not own the canonical BindingRecord schema. Standing does not own generic BindingRecordValid. Standing does not own the generic binding point, CycleRecord, Task Ledger, or F-5 objects. Standing contributes a prerequisite where applicable, and Core consumes that contribution through the BindingRecord standing disposition.
...
BindingFormed(β) ⟺
  ...
  StandingValid(σ)       ∧
  Reachable(τ)           ∧
  CoreBindingRecordValid(β)
```

This companion's historical BindingFormed vocabulary is now a standing-facing composite, not an ownership claim over BindingRecord. `CoreBindingRecordValid(β)` refers to the Core v5.14 predicate `BindingRecordValid(binding_record, historical_validation_context)`. BindingFormed deliberately does not include continuation. Binding and continuation remain distinct stages.

## Standing Applicability and Non-Applicability

Standing may be applicable or not applicable. Non-applicability must be positively determined. Omission is not non-applicability. Lack of evidence is not non-applicability. Failed validation is not non-applicability. Stale evidence is not non-applicability.

`standing_non_applicability_ref` points to Standing-owned evidence identifying the movement or movement class, domain constitution, reason standing does not apply, determining authority, effective time, provenance, and audit identity. This evidence may be carried by Standing-local structures and is not registered in CRC-SSR as a separate shared schema object.

Standing may use failure or validation outcomes corresponding to validation missing, validation failed, evidence unavailable, and evidence stale. These are non-bind conditions for the intended movement. They must not appear as completed BindingRecord standing dispositions. They may route to StandingFailureRecord, Hold, Escalate, PendingObligation, fresh validation, sovereign review, or domain review.

Core v5.14 consumes only two completed-record dispositions: `APPLICABLE_VALID` and `NOT_APPLICABLE`. `APPLICABLE_VALID` requires a historically valid `standing_ref`. `NOT_APPLICABLE` requires valid `standing_non_applicability_ref`. Missing, failed, stale, unavailable, or invalid required standing evidence makes Core BindingRecordValid false.

## Standing Timing for Binding

Proposal formation is not itself a standing-validity stage. The proposal may carry claimed or presented authority context, but proposal formation does not prove standing.

Resolution requires standing when the movement is standing-sensitive.

ActuationCommand issuance may inherit previously validated standing only when the evidence remains current, not expired, not revoked, not suspended, not affected by a material authority-context change, not affected by a domain-constitution change, and not subject to a standing-sensitive delay requiring revalidation. Fresh revalidation is required when any of those conditions fail.

Admission requires standing validation when admission is standing-sensitive.

Mutation requires fresh standing validation when standing applies and admission and mutation are not atomic, meaningful time has elapsed, authority context changed, standing evidence expired, revocation or suspension occurred, or the domain constitution requires it.

Binding requires no separate duplicate standing check after a mutation whose validation already included binding-time standing. BindingRecord consumes the historical standing evidence or non-applicability evidence.

If standing was valid at Resolution but fails before command, no command may issue. If standing was valid at command but fails before admission, the intended movement may not be admitted. If standing was valid at admission but fails before mutation, no intended mutation and no BindingRecord may form. If standing changes after binding, historical validity remains unchanged. Later revocation or changed authority requires a new governed movement. If standing is not applicable, positive non-applicability evidence is required.

## BindingFormed Correction

Binding occurs at valid applied governed mutation. BindingRecord records and proves that binding event. Standing contributes a prerequisite where applicable. Standing does not create binding.

Standing v1.7 evaluates standing as of the relevant historical stage. Later standing change does not silently rewrite a valid historical BindingRecord.
...
ContinuationIssued(o) ⟺
  ...
  GovernedOutcomeRecorded(o)                                      ∧
  ContinuationLineageReconstructable(o)                           ∧
  AllowedNextAffordancesDeclared(ContinuationState)               ∧
  PriorReasoningDiscarded(agent, cycle_id)
```

`o` ranges over whichever typed outcome the pipeline produced for this cycle, a BindingRecord, a HoldRecord, an EscalationState, a NonFormationReceipt, or a StandingFailureRecord (below). OperationalClosure's own use of ContinuationIssued(β), Part IV, is the special case of this general predicate where the outcome is specifically a binding, o = β; the predicate is not being narrowed there, only instantiated to the one outcome OperationalClosure is stated in terms of.

A **ContinuationState** is the Core-owned, substrate-issued continuity authority the substrate produces from a Resolution, and it is what authorizes the next ORSR cycle. Constitutional Runtime Computation v5.12 owns and defines the canonical ContinuationState schema and the Core issuance predicate `ContinuationStateIssuable`. This paper owns the standing-sensitive issuance and consumption predicates, including `ContinuationIssued` and `ContinuationGrounded`; it does not own, redefine, or replace the Core object, and it does not define a `derived_from_ref` field. The substrate constructs the next AgentObservation from that ContinuationState; the AgentObservation, Companion 0's governed Observe envelope, realized in the parent's Appendix A, is what crosses to the agent for the next cycle, carrying or referencing the ContinuationState through `continuation_state_ref`.

Every one of the five outcomes this paper's pipeline can produce, a bind, a Hold, a pending escalation, a formation failure, or a standing failure, may be followed by a new ContinuationState only when the substrate separately authorizes a next governed cycle. The failed, held, escalated, or otherwise unresolved movement does not bind, does not continue, and does not silently replay. Typed outcome lineage is reconstructed through `issuing_resolution_ref`, `cycle_record_ref`, `task_ledger_ref`, and the audit trace. The CycleRecord supplies the recorded verdict, `outcome_type`, and `outcome_ref`; ContinuationState itself does not carry a second outcome pointer. `AllowedNextAffordancesDeclared` requires that the new continuation authority name the Task Ledger-owned affordance set the substrate has newly declared reachable, since a continuation that tells the agent only what happened, without what it may now do, is not operationally useful, whatever else it contains. The ContinuationState carries current task status through the Core-owned field composition and Task Ledger semantics. It does not include, and cannot be constructed to include, any element of the agent's own private reasoning from the prior cycle. `PriorReasoningDiscarded(agent, cycle_id)` names the corresponding obligation on the agent side: whatever the agent privately inferred, hypothesized, or concluded within the cycle that just closed carries no constitutional force into the next one, whether or not the agent's own internal state happens to retain it. This is not a claim that the agent's internal memory is technically erased; it is the constitutional claim that it is not authoritative, exactly as the parent's own ORSR description already states for a resolution's IN_PROGRESS status and exactly as a second domain's own memory doctrine states in fuller detail (Part VIII): agent memory is not authoritative, substrate memory is authoritative, and ephemeral reasoning context dies at the cycle boundary regardless of verdict.

The critical case, and the one every prior companion's own worked examples have quietly assumed without stating, is the Hold path. §6a formalizes what happens to the record of a Hold: it is written, immutable, queryable, and resistant to replay. §6a does not formalize what happens to the agent after a Hold: does the agent's own reasoning about why the proposal was held carry forward, informing its next proposal, or must the agent's next proposal be built entirely from what the substrate reissues. This paper answers: the latter, and Part II's ContinuationGrounded conjunct is what makes the answer formally checkable rather than only stated here in prose. A Hold does not leave the agent free to privately reason about the cause and resubmit an improved proposal from its own continuing train of thought. If the task remains live, the substrate may authorize a separately governed remedial cycle with an AllowedNextAffordanceSet restricted to constitutionally reachable movements arising from the hold condition, such as waiting, evidence gathering, corrective resubmission, review, or sovereign-routed movement. Any improvement in the resubmission must trace to information the substrate itself surfaced, such as the HoldRecord's `cause` field, newly recorded provenance, or a reconstituted domain constitution, not to the agent's own private continuity of reasoning about the failure. The same rule applies to Escalate, non-formation, and standing failure: the failed or unresolved movement does not bind; no movement exists without substrate-issued continuation authority; and a new remedial cycle does not revive the failed movement. This is the precise boundary between a legitimate resubmission after genuine state change, which §6a's non-replay apparatus already governs correctly, and an illegitimate private continuation, which no apparatus governed until this part.
...
Possession of a structurally complete ContinuationState is insufficient for Standing consumption. The next Formation must be grounded in the substrate-issued AgentObservation and matching continuation lineage, and consumption remains checked against current task state, standing, cycle identity, Core issuance, and the audit trace.
...
**Recommended reading order:** Constitutional Runtime Computation v5.14; Constitutional Boundary Contracts v1.3; Constitutional Memory v2.2; Constitutional Retrieval v1.2; Constitutional Baselines v1.2; Constitutional Coherence v1.4; Constitutional Thresholds v1.4; Constitutional Standing v1.7 (this paper).
...
**ContinuationGrounded(π).** The proposal's declared reasoning basis, evidence references, prior-resolution reference, and claimed affordance reconstruct entirely from the most recent substrate-issued AgentObservation and matching ContinuationState lineage. The conjunct that makes Private Continuation After Verdict (P_stand4) formally detectable rather than described only in prose.
...
**BindingFormed(β).** FormationValid ∧ StandingValid ∧ Reachable(τ) ∧ CoreBindingRecordValid. After F-3 reconciliation this is a standing-facing composite, not an ownership claim over BindingRecord. Core v5.14 owns BindingRecordValid.

**BindingRecord.** The Core-owned durable constitutional record defined in Constitutional Runtime Computation v5.14. This paper supplies standing applicability, non-applicability, and historical validation evidence consumed by BindingRecord, but it does not own the canonical schema or generic predicate.
...
**ContinuationIssued(o).** The general predicate over every outcome the pipeline can produce, a BindingRecord, a HoldRecord, an EscalationState, a NonFormationReceipt, or a StandingFailureRecord: Core-owned ContinuationState issued, outcome lineage reconstructable through issuing Resolution, CycleRecord, Task Ledger, and audit trace, AllowedNextAffordancesDeclared, and PriorReasoningDiscarded. OperationalClosure's ContinuationIssued(β) is the special case where the outcome is a binding.

**ContinuationState.** The Core-owned, substrate-issued continuity authority the substrate produces from a Resolution; it authorizes the next ORSR cycle. Core v5.12 owns and defines the canonical schema and ContinuationStateIssuable predicate. This paper owns standing-sensitive issuance and consumption predicates, not the object schema. Task Ledger owns authoritative task state, CycleRecord, and AllowedNextAffordanceSet. The substrate constructs the next AgentObservation, Companion 0's governed Observe envelope, from ContinuationState, and that AgentObservation carries or references it through `continuation_state_ref`. No element of the agent's own private reasoning from the prior cycle survives into it, and no new Standing record is created.
...
*v1.7. F-3 BindingRecord reconciliation. Core ownership of BindingRecord and BindingRecordValid is formalized in Constitutional Runtime Computation v5.14. This paper clarifies that it owns StandingValid, StandingContext, standing applicability, positive standing non-applicability evidence, standing validation evidence, StandingFailureRecord, and timing or revalidation doctrine. It does not own the canonical BindingRecord schema, generic BindingRecordValid, generic binding point, CycleRecord, Task Ledger, or F-5 objects. StandingDisposition consumed by Core is limited to APPLICABLE_VALID and NOT_APPLICABLE. Missing, failed, stale, unavailable, or invalid required standing evidence is a non-bind condition for the intended movement, not a completed BindingRecord disposition. CRC-SSR v1.6 records Core ownership of BindingRecord. No em dashes.*

*v1.6. F-1 ContinuationState reconciliation. Core ownership is formalized and the canonical ContinuationState schema is published in Constitutional Runtime Computation v5.12. This paper clarifies that it owns `ContinuationIssued`, `ContinuationGrounded`, standing-sensitive consumption doctrine, StandingContext, and StandingFailureRecord, while Core owns the ContinuationState object schema and issuance predicate. `derived_from_ref` is not part of the Core schema, and this paper no longer states or implies a ContinuationState outcome-pointer field. Outcome lineage reconstructs through issuing Resolution, CycleRecord, Task Ledger, and audit trace. Hold, Escalate, non-formation, and standing failure do not continue the failed or unresolved movement; any remedial cycle is separately authorized by substrate-issued continuation authority. CRC-SSR v1.4 records the F-1 reconciliation. This is not a B.3.1 amendment back-reference. No em dashes.*
