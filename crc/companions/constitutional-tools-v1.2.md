...
### v1.2 Conceptual Architecture Paper, Companion 7 to Constitutional Runtime Computation v5.13
...
- oar_id: OAR-F003
  ...
  effect: Tool-result binding consumes the later F-3 BindingRecord reconciliation, but Tools may align to F-5 without completing F-3.
...
OAR-X005 is a closure target for this alignment, not a dependency declared under the Open Architecture Dependencies field.
...
Constitutional Runtime Computation v5.13 and Constitutional Actuation and Effect Admission v1.0 provide the generic F-5 apparatus that this paper now specializes. ToolInvocationProposal remains a proposal. ToolInvocationReachable(κ) determines whether the tool invocation proposal may receive an Emit verdict, but it does not itself create an ActuationCommand. A substrate tool actuator derives any tool-specific executable command from that authorized proposal, and that command must conform to Core v5.13's ActuationCommand.

Tool execution is an actuation attempt. ToolResultRecord specializes ActuationResult. ToolResultAdmissible(ρ) specializes ActuationResultAdmissible. A tool admission verdict conforms to ActuationAdmissionRecord. Any downstream state effect conforms to GovernedStateMutationRecord. Final tool binding remains dependent on later F-3 BindingRecord reconciliation.

This alignment does not expand Tools authority. Tools continues to own tool capability contracts, tool-specific risk taxonomy, tool-specific authorization constraints, and tool-specific result semantics. Tools does not own generic ActuationCommand, generic ActuationResult, generic admission, generic mutation, or BindingRecord.
...
A **ToolResultRecord** is the typed object a completed tool invocation produces, whatever its outcome, carrying: `tool_result_id`, `tool_invocation_ref` (the ToolInvocationProposal this result answers), `tool_ref` and `tool_version` (which registered tool and which version of it executed), `input_ref` (the grounded inputs actually used), `output_ref` and `output_type` (the typed result and its declared type), `execution_time`, `side_effects_observed` (what the tool reports having done, distinct from what it was authorized to do), `error_state`, `confidence_or_uncertainty` (the tool's own reported uncertainty, where applicable, distinct from the substrate's later admission judgment), `provenance_ref` (how this result's own origin is traceable), `admission_status`, `admission_target` (which future substrate surface, memory, evidence, Observe, external state, the result is proposed to affect), and `audit_ref`.

Under F-5, ToolResultRecord specializes ActuationResult. Its tool-local fields map to the generic requirements as follows: the originating tool-specific command supplies the ActuationCommand reference; each execution supplies an attempt ID; tool output and side_effects_observed supply observed effects; tool identity and registered capability supply source integrity; confidence_or_uncertainty supplies uncertainty; side_effects_observed and error_state supply partiality where applicable; tool operation class supplies irreversibility; tool replay and registry checks supply duplicate status; provenance_ref supplies provenance; and audit_ref supplies audit identity. External-effect references are required only where the tool operation actually reaches an external effect surface.

Existing local display values map to Core v5.13's generic AdmissionStatus values: PENDING maps to PENDING_ADMISSION, ADMITTED maps to ADMITTED, HELD maps to ADMISSION_HELD, and ESCALATED maps to ADMISSION_ESCALATED. Tools also supports the generic statuses PARTIALLY_ADMITTED, REJECTED, and VERIFICATION_REQUIRED. The generic canonical values are the F-5 values; local display terms do not create a competing admission vocabulary.
...
**ToolResultRecord.** The typed object a completed tool invocation produces: tool_result_id, tool_invocation_ref, tool_ref, tool_version, input_ref, output_ref, output_type, execution_time, side_effects_observed, error_state, confidence_or_uncertainty, provenance_ref, admission_status, admission_target, audit_ref. Under F-5, it specializes ActuationResult.

**ToolResultAdmissible(ρ).** InvocationAuthorized ∧ OutputTyped ∧ OutputWithinDeclaredScope ∧ ObservedEffectsMatched ∧ SourceIntegrityVerified ∧ UncertaintyAnnotated ∧ AdmissionTargetPermitted ∧ DownstreamEffectDeclared. Separates a tool call's emission from its result's independent admission; ObservedEffectsMatched, added on first-pass review, checks reported side effects against what the invocation declared. Under F-5, it specializes ActuationResultAdmissible.
...
*v1.2. F-5 Phase 1 alignment. Aligns Constitutional Tools with Constitutional Runtime Computation v5.13 and Constitutional Actuation and Effect Admission v1.0. Clarifies that ToolInvocationProposal remains a proposal; ToolInvocationReachable does not itself produce ActuationCommand; a substrate tool actuator derives a tool-specific command conforming to ActuationCommand; tool execution is an actuation attempt; ToolResultRecord specializes ActuationResult; ToolResultAdmissible specializes ActuationResultAdmissible; tool admission conforms to ActuationAdmissionRecord; downstream state effects conform to GovernedStateMutationRecord; and final tool binding remains dependent on later F-3 BindingRecord reconciliation. Maps local tool admission display values to the generic F-5 AdmissionStatus values, adds support for PARTIALLY_ADMITTED, REJECTED, and VERIFICATION_REQUIRED, and preserves Tools ownership of tool-specific contracts, risks, authorization constraints, and result semantics without expanding Tools authority over generic actuation, admission, mutation, or BindingRecord. No em dashes.*
