...
### v5.13 Conceptual Architecture Paper
...
### F-5 Open Architecture Declaration
...
Observe -> Reason -> Submit -> Resolve -> Issue ActuationCommand -> Attempt -> Record ActuationResult -> Validate -> Admit or Reject -> Apply Governed State Mutation -> Finalize Binding -> Record Cycle -> Update Task Ledger -> Issue ContinuationState -> Construct AgentObservation.

ORSR remains the top-level name for the governed loop. Deterministic internal branches may make some post-command stages atomic, but every constitutional distinction must remain reconstructable: proposal is not authorization, authorization is not command, command is not attempt, attempt is not success, result validity is not result admission, admission is not mutation, mutation is not binding finalization, audit capture is not binding, and external occurrence is not governed causal force.
...
### Binding Point and Dependency Contract

Binding occurs when an authorized and admitted movement acquires governed causal force through a valid governed-state mutation. Binding may be atomic with mutation or recorded immediately after mutation, but it must never precede mutation. External occurrence alone does not bind. Resolution alone does not bind. Admission alone does not bind. Audit alone does not bind. Compensation, rollback, accounting, and obligation creation may bind as distinct movements when separately admitted and applied.

F-5 does not finalize the complete BindingRecord schema. It requires a future BindingRecord to reference or reconstruct `resolution_ref`, `actuation_command_ref`, `actuation_result_ref`, `admission_record_ref`, `state_mutation_ref`, task identity, cycle identity, authority context, standing basis where applicable, domain constitution, admitted effect scope, binding time, provenance, and audit identity.

F-3 remains responsible for final canonical BindingRecord schema, final owner reconciliation, BindingRecordValid, Standing alignment, and registry settlement.

### Canonical Actuation Write Order
...
References should flow forward by creation order. Identifiers may be preallocated. CycleRecord may point to outcome without requiring outcome to point back. Audit lineage reconstructs order. Failed, rejected, unknown, and partial branches omit steps that did not lawfully occur. No BindingRecord exists for an intended movement that never gained governed causal force.
...
The POC's BindingRecord and state-update ordering is illustrative and non-canonical after v5.13. Any apparent BindingRecord-before-state-update sequence in this POC does not override the Section 4 F-5 rule that binding must never precede a valid governed-state mutation and that the final BindingRecord schema remains unsettled under F-3.
