...
### v5.14 Conceptual Architecture Paper
...
ContinuationState is a Core-owned constitutional object. It is issued only by the substrate, authorizes one specific next ORSR cycle, and is derived from the most recently completed governed cycle and the authoritative Task Ledger state. It does not duplicate the Task Ledger, and Constitutional Standing does not own or redefine it. Structurally complete possession alone does not create valid continuation authority; a ContinuationState is valid only when substrate-issued and valid under the Core issuance predicate below.

### Canonical ContinuationState

The canonical ContinuationState is the Core-owned constitutional object that carries next-cycle continuation authority. It has exactly the following required fields and no conditional or optional fields.

| Field | Type | Constitutional function |
|---|---|---|
| `continuation_state_id` | ContinuationStateRef | Uniquely identifies the substrate-issued continuation authority |
| `task_id` | TaskRef | Identifies the governed task authorized to continue |
| `cycle_id` | CycleRef | Identifies the next ORSR cycle authorized by this ContinuationState |
| `issuing_resolution_ref` | ResolutionRef | Identifies the Resolution that produced this ContinuationState |
| `task_ledger_ref` | TaskLedgerRef | Identifies the authoritative Task Ledger for the task |
| `cycle_record_ref` | CycleRecordRef | Identifies the Task Ledger-owned CycleRecord for the just-closed cycle |
| `goal_status` | GoalStatusEnum | Carries the governed task status into the next cycle |
| `allowed_next_affordance_set_ref` | AllowedNextAffordanceSetRef | References the Task Ledger-owned AllowedNextAffordanceSet issued for `cycle_id` |
| `authority_context_ref` | AuthorityContextRef | Pins the authority context for the authorized next cycle |
| `domain_constitution_ref` | DomainConstitutionRef | Pins the domain constitution governing the authorized next cycle |
| `provenance_frontier_ref` | ProvenanceFrontierRef | Pins the provenance frontier used for changed-condition and replay analysis |

`continuation_state_id` must be unique in the audit trace.

`task_id` must resolve to the task owned by `task_ledger_ref`.

`cycle_id` identifies the next ORSR cycle authorized by this ContinuationState. It does not identify the cycle just resolved. For the proposal submitted from the AgentObservation constructed from this ContinuationState:

```
ContinuationState.cycle_id == AgentObservation.cycle_id == TransitionProposal.cycle_id
```

`issuing_resolution_ref` identifies the Resolution that produced this ContinuationState. It must equal the next `AgentObservation.issuing_resolution_ref`, and the proposal submitted from that observation must carry a matching `prior_resolution_ref`. `prior_resolution_ref` is not a ContinuationState field. The Appendix A proof-of-concept field named `prior_resolution_ref` maps conceptually to canonical `issuing_resolution_ref`.

`task_ledger_ref` identifies the authoritative Task Ledger for the task. It does not embed the ledger, and it must resolve to the ledger identified by `task_id`.

`cycle_record_ref` identifies the Task Ledger-owned CycleRecord for the just-closed cycle. The CycleRecord must have been written before continuation issuance. The CycleRecord supplies the recorded verdict, `outcome_type`, `outcome_ref`, and audit lineage. Specific typed outcome lineage is reconstructed through the CycleRecord and audit trace; ContinuationState carries no second outcome pointer.

`goal_status` carries the governed task status into the next cycle. Constitutional Task Ledger owns operational task-status semantics. Constitutional Boundary Contracts owns `GoalStatusEnum`. Core owns the ContinuationState field composition. The required invariants are:

```
ContinuationState.goal_status == TaskLedger.status
```

at the state associated with `cycle_record_ref`, and:

```
AgentObservation.goal_status == ContinuationState.goal_status
```

`allowed_next_affordance_set_ref` references the Task Ledger-owned AllowedNextAffordanceSet issued for `cycle_id`. It does not embed `allowed_next_affordances`. `AgentObservation.allowed_next_affordances` is the governed projection of this set. ProposalContract capability validation checks the proposal against that projection, and the projection must trace to the same AllowedNextAffordanceSet.

`authority_context_ref` pins the authority context for the authorized next cycle. StandingContext may supply standing-related component semantics, but Standing does not own or redefine this Core field. The authority context must be current for `cycle_id`.

`domain_constitution_ref` pins the domain constitution governing the authorized next cycle. It must match the constitution used by the substrate during issuance.

`provenance_frontier_ref` pins the provenance frontier used for changed-condition and replay analysis. Changed-condition claims must be reconstructable through that frontier.

Hold or Escalate does not authorize execution or continuation of the held or escalated transition. The substrate may issue a new ContinuationState only if it authorizes a separately governed next cycle. The new AllowedNextAffordanceSet must be restricted to constitutionally reachable movements arising from the hold or escalation condition, including waiting, evidence gathering, corrective resubmission, review, or sovereign-routed movement where those movements are authorized. The prior unresolved transition does not bind and does not silently replay.

A failed Formation or proposal does not bind. The failed movement does not continue. No next movement exists unless the substrate issues new continuation authority. A new ContinuationState may be issued only as separately authorized remedial continuation for a live task, and the remedial cycle does not revive the failed movement.

Core owns the issuance predicate `ContinuationStateIssuable(c)`:

```
ContinuationStateIssuable(c) iff
  SchemaComplete(c)                         and
  SubstrateIssued(c)                        and
  IssuingResolutionValid(c)                 and
  NextCycleAuthorized(c)                    and
  TaskLedgerConsistent(c)                   and
  CycleRecordConsistent(c)                  and
  GoalStatusConsistent(c)                   and
  AllowedNextAffordanceSetValid(c)          and
  AuthorityContextCurrent(c)                and
  DomainConstitutionPinned(c)               and
  ProvenanceFrontierValid(c)                and
  AuditReconstructable(c)
```

A structurally complete ContinuationState is not valid continuation authority unless it was substrate-issued and satisfies `ContinuationStateIssuable`. Core does not define a separate `ContinuationStateCurrent` predicate. Currentness remains distributed across Standing consumption doctrine, Task Ledger current-state checks, cycle identity, and audit state.

Standing retains `ContinuationIssued` and `ContinuationGrounded`. Standing governs standing-sensitive consumption and private-continuation exclusion. Core governs the object schema and issuance. Standing does not prove Core issuance merely by evaluating structural completeness.

Core owns the ContinuationState field composition. Constitutional Task Ledger owns TaskLedger, CycleRecord, and AllowedNextAffordanceSet. Constitutional Boundary Contracts owns GoalStatusEnum. Constitutional Standing owns standing predicates, StandingContext, and StandingFailureRecord. Referencing these objects does not transfer their ownership to Core.

F-1 does not settle BindingRecord. `CycleRecord.outcome_ref` may eventually reference a BindingRecord, but that remains an adjacent F-3 dependency. No BindingRecordRef appears in the canonical schema. Admitted-effect and actuation-result lineage remains under provisional F-5. No effect-result field appears in the canonical schema.
...
### F-3 Open Architecture Declaration

Open Architecture Dependencies:

- oar_id: OAR-F005
  status: SPECIFIED
  relationship: evidence_dependency
  materiality: non-blocking
  effect: F-5 supplies the published actuation, admission, mutation, and binding dependency contract used by F-3 without making F-5 lifecycle closure a prerequisite to F-3 publication.

Source OAR Entry:

- oar_id: OAR-F003
  status: BLOCKED
  role: governing source finding and F-3 restart target

OAR-F003 is not listed as its own dependency. The F-5 apparatus entries are not independent dependencies for this package.

### F-5 Evidence Note

F-5 is supporting public evidence for this F-3 reconciliation. Core v5.13,
Constitutional Actuation and Effect Admission v1.0, Tools v1.2, Task Ledger
v1.3, and CRC-SSR v1.5 published the actuation, admission, mutation, and binding
dependency contract consumed here. F-5 lifecycle closure is not a prerequisite
to F-3 publication, and the F-5 apparatus entries are not independent Open
Architecture Dependencies for this package.

### Canonical Actuation Sequence

Resolution(Emit) authorizes the substrate to consider issuance of an ActuationCommand. It does not execute the movement, prove an attempt, prove success, admit the result, mutate governed state, or create binding.

The expanded ORSR sequence is:

Observe -> Reason -> Submit -> Resolve -> Issue ActuationCommand -> Attempt -> Record ActuationResult -> Validate -> Admit or Reject -> Apply Governed State Mutation -> Record BindingRecord -> Record Cycle -> Update Task Ledger -> Issue ContinuationState -> Construct AgentObservation.

ORSR remains the top-level name for the governed loop. Deterministic internal branches may make some post-command stages atomic, but every constitutional distinction must remain reconstructable: proposal is not authorization, authorization is not command, command is not attempt, attempt is not success, result validity is not result admission, admission is not mutation, mutation is not binding, BindingRecord persistence is not causal force by assertion, audit capture is not binding, and external occurrence is not governed causal force.

### Actuator Interface

An actuator is a substrate-owned constitutional role or interface through which an authorized command may be attempted. The agent does not own the actuator and cannot transform its own proposal into an executable command. Tools may be actuator adapters or endpoints. Memory writes may be internal actuation branches. The governed mutation engine applies admitted changes. Observation construction remains sensing, not actuation. Ordinary audit append is evidentiary support, not binding.

### Canonical ActuationCommand

ActuationCommand is the Core-owned constitutional object by which the substrate scopes and issues actuation authority after adjudicative authorization.

Required fields:

| Field | Type | Constitutional function |
|---|---|---|
| `actuation_command_id` | ActuationCommandRef | uniquely identifies the substrate-issued command |
| `task_id` | TaskRef | identifies the governed task |
| `cycle_id` | CycleRef | identifies the cycle for which the command is issued |
| `proposal_ref` | TransitionProposalRef | references the proposal whose adjudication produced the command authority |
| `resolution_ref` | ResolutionRef | references the Emit or equivalent authorized sovereign route |
| `authority_context_ref` | AuthorityContextRef | pins the authority context for command issuance |
| `domain_constitution_ref` | DomainConstitutionRef | pins the domain constitution governing the command |
| `actuator_ref` | ActuatorRef | identifies the substrate-owned actuator role or endpoint |
| `actuation_type` | ActuationType | classifies the actuation branch |
| `authorized_scope` | AuthorizedScope | states the maximum effect scope authorized by the substrate |
| `constraints` | ConstraintSet | records command constraints, including branch and safety constraints |
| `issued_at` | Timestamp | records command issuance time |
| `expires_at` | Timestamp | records command expiry |
| `provenance_ref` | ProvenanceRef | supplies command provenance |
| `audit_id` | AuditEventRef | identifies durable audit capture for command issuance |

Conditional fields:

| Field | Type | Condition |
|---|---|---|
| `target_ref` | TargetRef | required when the branch has a target state, endpoint, account, recipient, store, tool, or other object |
| `idempotency_key` | IdempotencyKey | required when the branch may retry, may duplicate, or reaches an external or non-idempotent surface |

An ActuationCommand is substrate-issued, one-use, and expiring. Its scope may narrow but may never expand the authorized scope. Raw agent proposal content must not be forwarded as executable authority without substrate construction and scope filtering. Batching is allowed only when authorized and decomposable by effect. Command identity may be preallocated, but use requires current validity.

Core owns `ActuationCommandIssuable(command, resolution, substrate_state)`, meaning the substrate is constitutionally permitted to issue the command. The predicate requires substrate issuance, a valid Resolution reference, Emit or equivalent authorized sovereign route, task and cycle alignment, authority-context currency, domain-constitution currency, standing validity where applicable, scope non-expansion, target validity, branch conformance, idempotency compliance, and no private-continuation bypass.

Core owns `ActuationCommandValid(command, substrate_state)`, meaning an issued command remains valid for use at the current state. The predicate requires command freshness, one-use status, current authority context, current domain constitution, target validity, branch conformance, idempotency compliance, no superseding Hold or Escalate, and no private-continuation bypass. Failure of either predicate halts actuation.

### Actuation Attempt Semantics

An actuation attempt is substrate-owned execution of an ActuationCommand. Core creates no canonical ActuationAttemptRecord. Attempt identity is represented through `attempt_id`, `actuation_command_ref`, actuator identity, audit lineage, and ActuationResult provenance. Retries create distinct attempt identifiers. A retry requires explicit bounded safe-retry authority in the command under a ratified idempotency rule, or fresh constitutional authority.

### Canonical ActuationResult

ActuationResult is the Core-owned constitutional object recording the observed result of an actuation attempt.

Required fields:

| Field | Type | Constitutional function |
|---|---|---|
| `actuation_result_id` | ActuationResultRef | uniquely identifies the result |
| `actuation_command_ref` | ActuationCommandRef | links the result to the command attempted |
| `attempt_id` | AttemptRef | identifies the specific attempt |
| `result_status` | ActuationResultStatus | classifies the attempt result |
| `observed_effects` | ObservedEffectSet | records reported or observed effects; may be empty |
| `source_integrity` | SourceIntegrityRecord | records source and actuator integrity evidence |
| `uncertainty` | UncertaintyRecord | carries result uncertainty |
| `partiality` | PartialityRecord | records whether the result was complete or partial |
| `irreversibility` | IrreversibilityRecord | records reversibility or irreversibility posture |
| `duplicate_status` | DuplicateStatus | records duplicate-attempt or duplicate-effect status |
| `started_at` | Timestamp | records attempt start |
| `provenance_ref` | ProvenanceRef | supplies result provenance |
| `audit_id` | AuditEventRef | identifies durable audit capture for result recording |

Conditional fields:

| Field | Type | Condition |
|---|---|---|
| `external_effect_ref` | ExternalEffectRef | required when an external effect may have occurred |
| `compensation_status` | CompensationStatus | required when compensation is relevant |
| `completed_at` | Timestamp | required when completion time is known |

Generic result-status values are exactly `NOT_ATTEMPTED`, `COMPLETED`, `FAILED`, `PARTIALLY_COMPLETED`, `TIMED_OUT`, and `RESULT_UNKNOWN`. Compensation is not an original result status.

Core owns `ActuationResultValid(result, command, substrate_state)`, meaning the result is authentic, scoped, and reconstructable. The predicate checks command identity, attempt identity, authorized actuator source, authenticity, scope conformance, timestamp ordering, duplicate detection, uncertainty representation, partial-effect representation, external-effect evidence where applicable, provenance completeness, and no silent agent-generated result synthesis. An ActuationResult may be valid and still be inadmissible.

### Canonical ActuationAdmissionRecord

ActuationAdmissionRecord is the Core-owned constitutional object recording whether and how an ActuationResult may affect governed state.

Required fields:

| Field | Type | Constitutional function |
|---|---|---|
| `admission_record_id` | ActuationAdmissionRecordRef | uniquely identifies the admission decision |
| `actuation_result_ref` | ActuationResultRef | references the result under admission review |
| `admission_status` | AdmissionStatus | records the generic admission status |
| `admitted_effect_scope` | EffectScope | records the admitted scope; may be empty |
| `rejected_effect_scope` | EffectScope | records the rejected scope; may be empty |
| `authority_context_ref` | AuthorityContextRef | pins the authority context for admission |
| `domain_constitution_ref` | DomainConstitutionRef | pins the domain constitution for admission |
| `decision_basis` | DecisionBasisRef | records the basis for admission, rejection, hold, escalation, or verification |
| `decided_at` | Timestamp | records decision time |
| `provenance_ref` | ProvenanceRef | supplies admission provenance |
| `audit_id` | AuditEventRef | identifies durable audit capture for admission |

Conditional fields:

| Field | Type | Condition |
|---|---|---|
| `standing_ref` | StandingRef | required when standing-sensitive admission is evaluated |
| `verification_ref` | VerificationRef | required when verification is used or required |
| `obligation_refs` | List[PendingObligationRef] | required when admission creates or updates obligations |
| `admitted_at` | Timestamp | required when an effect is admitted |

Generic AdmissionStatus values are exactly `PENDING_ADMISSION`, `ADMITTED`, `PARTIALLY_ADMITTED`, `REJECTED`, `ADMISSION_HELD`, `ADMISSION_ESCALATED`, and `VERIFICATION_REQUIRED`. Core does not use bare `HELD` or `ESCALATED` as generic AdmissionStatus values.

Core owns `ActuationResultAdmissible(result, substrate_state, authority_context)`. Validity does not prove admissibility. Admission is separately adjudicated. Partial admission is lawful only when admitted and rejected scopes are explicit. Rejected scope cannot mutate as the intended movement. Admission-stage routing is not the original Resolution verdict. `ADMISSION_ESCALATED` requires authority engagement but does not pre-establish authority conclusion.

Admission consequences:

| Status | Consequence |
|---|---|
| `PENDING_ADMISSION` | no mutation, no binding, admission not terminal |
| `ADMITTED` | mutation permitted within admitted scope; binding possible only after valid mutation |
| `PARTIALLY_ADMITTED` | mutation permitted only within admitted scope; rejected or unresolved scope may create obligations; remedial movement requires fresh authority unless already lawfully authorized |
| `REJECTED` | no intended mutation and no binding of the intended movement; separate accounting or obligation mutation may occur |
| `ADMISSION_HELD` | no intended mutation, no binding, unresolved condition remains, PendingObligation may be required |
| `ADMISSION_ESCALATED` | no intended mutation until authorized authority resolves, no binding, sovereign or domain engagement required, conclusion remains open |
| `VERIFICATION_REQUIRED` | no intended mutation, no binding, verification route must complete |

### Canonical GovernedStateMutationRecord

GovernedStateMutationRecord is the Core-owned constitutional object recording an admitted mutation to governed state.

Required fields:

| Field | Type | Constitutional function |
|---|---|---|
| `governed_state_mutation_id` | GovernedStateMutationRecordRef | uniquely identifies the mutation record |
| `admission_record_ref` | ActuationAdmissionRecordRef | references the admission authorizing the mutation |
| `actuation_result_ref` | ActuationResultRef | references the result whose admitted scope is being applied |
| `target_state_ref` | GovernedStateRef | identifies the governed state target |
| `prior_state_ref` | GovernedStateRef | pins the prior state |
| `resulting_state_ref` | GovernedStateRef | pins the resulting state |
| `mutation_type` | MutationType | classifies the mutation |
| `mutation_scope` | EffectScope | records the mutation scope |
| `authority_context_ref` | AuthorityContextRef | pins the authority context |
| `domain_constitution_ref` | DomainConstitutionRef | pins the governing domain constitution |
| `atomicity` | AtomicityClass | records atomicity posture |
| `provenance_ref` | ProvenanceRef | supplies mutation provenance |
| `audit_id` | AuditEventRef | identifies durable audit capture for mutation |
| `applied_at` | Timestamp | records application time |

Conditional fields:

| Field | Type | Condition |
|---|---|---|
| `rollback_status` | RollbackStatus | required when rollback is available, attempted, or required |
| `compensation_ref` | CompensationRef | required when compensation is available, attempted, or required |

Mutation may not exceed admitted scope. A rejected intended movement produces no intended mutation. Obligation creation may be a governed mutation. External-effect accounting may be a governed mutation. Append-only stores may use prior and resulting positions, roots, or hashes. Audit append is not automatically a governed mutation. A failed mutation attempt is not a successful GovernedStateMutationRecord. Partial mutation must be explicitly represented and routed to rollback, compensation, or escalation.

Core owns `StateMutationReachable(mutation, admission, substrate_state)`, meaning the proposed mutation may lawfully be applied. Core owns `GovernedStateMutationValid(mutation, admission, substrate_state)`, meaning the record validly represents an admitted applied mutation. These predicates check admitted scope, correct target, prior-state match, authorized mutation engine, authority context, domain constitution, standing where applicable, atomicity, branch constraints, rollback rules, scope containment, provenance, idempotency, and duplicate mutation prevention.

### Binding Point

A movement binds at the valid governed-state mutation that applies its admitted effect scope to governed state. A BindingRecord does not create binding. It does not supply causal force by assertion. It records and proves the binding event, and may be persisted atomically with the mutation or after it. A completed BindingRecord must never precede the mutation it records.

Resolution alone does not bind. Admission alone does not bind. External occurrence alone does not bind. Audit alone does not bind. BindingRecord assertion alone does not bind. Binding occurs at a valid applied GovernedStateMutationRecord. BindingRecord is evidentiary and declarative. A missing BindingRecord does not erase a completed valid governed mutation. Missing binding evidence blocks ordinary cycle closure pending reconciliation.

Where this paper uses binding finalization after v5.14, the phrase means non-constitutive evidentiary completion through a valid BindingRecord. It does not mean that record persistence creates the binding.

### Canonical BindingRecord

BindingRecord is the Core-owned durable constitutional record proving that one admitted movement acquired governed causal force through one valid governed-state mutation.

The lawful unit is one binding event, one explicit bound effect scope, and exactly one GovernedStateMutationRecord reference. A single BindingRecord must not cover multiple independent governed mutations. Independent bound mutations require independent BindingRecords. A single atomic GovernedStateMutationRecord may support one BindingRecord containing a decomposed bound scope only when the mutation record itself preserves that atomic decomposition.

Required fields:

| Field | Type | Constitutional function |
|---|---|---|
| `binding_record_id` | BindingRecordRef | uniquely identifies the Core-owned binding record |
| `task_id` | TaskRef | identifies the governed task |
| `cycle_id` | CycleRef | identifies the cycle in which binding occurred |
| `resolution_ref` | ResolutionRef | references the Resolution or authorized sovereign route |
| `admission_record_ref` | ActuationAdmissionRecordRef | references the admission permitting mutation |
| `state_mutation_ref` | GovernedStateMutationRecordRef | references exactly one applied governed-state mutation |
| `authority_context_ref` | AuthorityContextRef | pins the authority context effective at binding time |
| `domain_constitution_ref` | DomainConstitutionRef | pins the domain constitution effective at binding time |
| `bound_effect_scope` | EffectScope | records the effect scope that bound |
| `binding_type` | BindingType | classifies the bound movement |
| `standing_disposition` | StandingDisposition | records applicable valid standing or positive non-applicability |
| `binding_time` | Timestamp | records when governed causal force was acquired through mutation |
| `recording_mode` | RecordingMode | records contemporaneous or reconstructed persistence |
| `recorded_at` | Timestamp | records durable BindingRecord persistence time |
| `provenance_ref` | ProvenanceRef | supplies binding provenance |
| `audit_id` | AuditEventRef | identifies durable audit capture for the record |

Conditional fields:

| Field | Type | Condition |
|---|---|---|
| `proposal_ref` | TransitionProposalRef | required for proposal-originated movements |
| `actuation_command_ref` | ActuationCommandRef | required when the movement was command-mediated |
| `actuation_result_ref` | ActuationResultRef | required when the movement was result-mediated |
| `standing_ref` | StandingRef | required only when `standing_disposition = APPLICABLE_VALID` |
| `standing_non_applicability_ref` | StandingNonApplicabilityRef | required only when `standing_disposition = NOT_APPLICABLE` |
| `unbound_effect_scope` | EffectScope | required for partial admission, partial mutation, or explicitly scoped remainder |
| `reconstruction_ref` | BindingClosureRepairRef | required only when `recording_mode = RECONSTRUCTED` |

`state_mutation_ref` is required universally. Each BindingRecord references exactly one GovernedStateMutationRecord. `admission_record_ref` is required universally. Every completed bind must identify the admission that permitted its mutation. `authority_context_ref` and `domain_constitution_ref` are required universally and identify the pinned context effective at binding time.

`bound_effect_scope` must be contained within the admitted effect scope, valid mutation scope, and authorized scope. `binding_time` is the time governed causal force was acquired through the applied mutation and must not be used as record persistence time. `recorded_at` is the time the BindingRecord was durably persisted.

RecordingMode values are exactly `CONTEMPORANEOUS` and `RECONSTRUCTED`. `RECONSTRUCTED` requires `reconstruction_ref`, which must identify the Task Ledger repair obligation or durable repair lineage authorizing and evidencing reconstruction. `reconstruction_ref` is prohibited for contemporaneous records.

StandingDisposition values are exactly `APPLICABLE_VALID` and `NOT_APPLICABLE`. `APPLICABLE_VALID` requires `standing_ref` and prohibits `standing_non_applicability_ref`; it means standing applied to the movement, standing was valid at the standing-sensitive mutation and binding point, and the reference is part of the pinned historical validation context. `NOT_APPLICABLE` requires `standing_non_applicability_ref` and prohibits `standing_ref`; it means Standing doctrine positively determines that standing did not apply, with non-applicability evidenced rather than inferred from omission. Exactly one valid combination must be present. Null, omission, missing evidence, failed validation, stale evidence, or unavailable evidence must not mean non-applicability.

`VALIDATION_MISSING`, `VALIDATION_FAILED`, `EVIDENCE_UNAVAILABLE`, and `EVIDENCE_STALE` are not BindingRecord standing values. Those conditions prohibit completed BindingRecord formation for the intended movement.

BindingType values are exactly `INTENDED_MOVEMENT`, `OBLIGATION_CREATION`, `ACCOUNTING`, `ROLLBACK`, `COMPENSATION`, and `NO_OP_GOVERNED_FACT`. NonFormation, StandingFailure, Refusal, Hold, Escalate, failed attempt, rejected admission, and verification pending are not BindingType values. Core defines no BindingRecord status field; a BindingRecord exists only for a completed bind.

A BindingRecord may use `NO_OP_GOVERNED_FACT` only if the no-op is itself an admitted and applied governed state fact. A skipped or failed mutation is not a no-op bind. An admitted and applied obligation-creation mutation may bind as `OBLIGATION_CREATION`. An admitted accounting mutation may bind as `ACCOUNTING`. Rollback and compensation are later movements. Each receives its own BindingRecord only if separately authorized, admitted, and applied. They do not erase the original BindingRecord. For an externally occurred but inadmissible effect, no BindingRecord exists for the intended movement. A separate accounting or obligation movement may bind if admitted and applied.

### BindingRecordValid

Core owns `BindingRecordValid(binding_record, historical_validation_context)`.

`BindingRecordValid` determines whether the record validly proves that a historical binding event occurred through the constitutionally required chain effective at binding time. It does not determine current authority for another movement, current standing, current enforceability by itself, or whether later rollback or compensation occurred.

`historical_validation_context` is the pinned historical evidence effective at binding time, including Resolution, ActuationCommand where applicable, ActuationResult where applicable, ActuationAdmissionRecord, GovernedStateMutationRecord, standing evidence or standing non-applicability evidence, authority context, domain constitution, provenance, durable audit evidence, task identity, and cycle identity. Historical binding must not be validated solely against current substrate state.

The predicate requires substrate-issued BindingRecord identity, valid Resolution or authorized sovereign route, valid ActuationCommand where applicable, valid ActuationResult where applicable, valid ActuationAdmissionRecord, admission status permitting the bound mutation, valid GovernedStateMutationRecord, successful mutation application, mutation scope contained within admitted scope, bound effect scope contained within mutation scope, bound effect scope contained within authorized scope, task alignment, cycle alignment, authority-context alignment, domain-constitution alignment, valid historical standing treatment, binding time at mutation application time or constitutionally equivalent transaction time, `recorded_at` not earlier than `binding_time`, correct `recording_mode`, `reconstruction_ref` present only when required, provenance completeness, durable audit capture, duplicate BindingRecord prevention, no unresolved Hold for the bound scope, no unresolved Escalate for the bound scope, no unresolved `ADMISSION_HELD`, no unresolved `ADMISSION_ESCALATED`, no unresolved `VERIFICATION_REQUIRED`, no standing failure for the bound scope, and no private continuation bypass.

The standing validity alternative must be exactly one of the following. For applicable standing, `standing_disposition = APPLICABLE_VALID`, `standing_ref` is historically valid, and `standing_non_applicability_ref` is absent. For standing not applicable, `standing_disposition = NOT_APPLICABLE`, `standing_non_applicability_ref` is historically valid, and `standing_ref` is absent. Any missing, failed, stale, unavailable, or invalid required standing evidence makes `BindingRecordValid` false.

If `BindingRecordValid` fails, the record must not be treated as valid proof of binding, ordinary CycleRecord Bind closure must not rely on it, continuation must not proceed as though closure were complete, and the failure must enter repair, reconciliation, Hold, or Escalate handling. Invalid record evidence must not erase a mutation that independently occurred. The system must distinguish invalid record from absent mutation.

Later standing loss does not retroactively invalidate a historically valid bind. Later constitutional changes do not silently rewrite historical validity. Current enforceability is a separate question. Revocation, reversal, rollback, and compensation are later governed movements. Historical BindingRecord evidence is append-only.

### Missing BindingRecord Reconciliation

When a valid GovernedStateMutationRecord was applied, binding occurred, and BindingRecord persistence failed or is uncertain, the completed mutation remains historically real and the bind is not erased. No ordinary CycleRecord Bind may be written until BindingRecord exists. Ordinary cycle closure is blocked. Ordinary ContinuationState is blocked. A mandatory Task Ledger repair obligation is created. Restricted repair or escalation continuation may issue only if separately authorized. Reconstruction is mandatory unless sovereign or domain doctrine explicitly determines another lawful resolution.

Reconstruction must validate against GovernedStateMutationRecord, durable mutation audit capture, admission lineage, authority lineage, standing evidence, task and cycle identity, duplicate detection, and prior BindingRecord write-attempt evidence where available.

A reconstructed BindingRecord must use `recording_mode = RECONSTRUCTED` and include `reconstruction_ref`, original `binding_time`, later `recorded_at`, complete provenance, and durable audit identity. Reconstruction cannot invent a mutation, expand bound scope, repair invalid standing, repair invalid admission, manufacture authority, or overwrite an earlier valid BindingRecord.

Before reconstruction, the substrate must search by `state_mutation_ref`, `admission_record_ref`, `task_id`, `cycle_id`, bound effect scope, audit lineage, and prior record-write evidence. If conflicting binding claims exist, ordinary continuation halts, evidence is preserved, repair obligation is created or preserved, and escalation follows domain doctrine.

### Canonical Binding Write Order

The canonical successful write order is:

1. Resolution
2. ActuationCommand
3. attempt identity
4. ActuationResult
5. ActuationAdmissionRecord
6. GovernedStateMutationRecord
7. BindingRecord
8. CycleRecord
9. Task Ledger update
10. ContinuationState
11. AgentObservation

Binding occurs at step 6. BindingRecord is ordinarily written at step 7. Steps 6 and 7 may share one atomic transaction. Step 7 must never occur as a completed valid record before step 6. If step 6 succeeds and step 7 fails, repair handling applies. CycleRecord Bind cannot point to a missing BindingRecord.

For `CycleRecord.outcome_type = Bind`, `outcome_ref` points directly to BindingRecord. BindingRecord does not point back to CycleRecord. The one-way reference preserves creation order. BindingRecord and CycleRecord may share a transaction only if the logical reference order remains non-circular. Non-bind outcomes do not create BindingRecord.

Hold creates no BindingRecord. Escalate creates no BindingRecord unless later authority produces a separate admitted mutation. NonFormation creates no BindingRecord. StandingFailure creates no BindingRecord. Failed ActuationResult, `RESULT_UNKNOWN`, rejected admission, `ADMISSION_HELD`, `ADMISSION_ESCALATED`, `VERIFICATION_REQUIRED`, and failed mutation create no BindingRecord for the intended movement. Partial mutation creates a BindingRecord only for valid admitted applied scope. Rollback and compensation are separate later movements.

### Formal Binding Invariants

1. Resolution alone does not bind.
2. Admission alone does not bind.
3. External occurrence alone does not bind.
4. Binding occurs at a valid applied governed-state mutation.
5. BindingRecord records and proves binding; it does not create binding.
6. BindingRecord never precedes the mutation it records.
7. BindingRecord never expands admitted scope.
8. BindingRecord never repairs invalid standing retroactively.
9. Standing applicability or non-applicability must be positively typed.
10. A completed BindingRecord may carry only APPLICABLE_VALID or NOT_APPLICABLE standing disposition.
11. Missing, failed, unavailable, or stale required standing evidence prohibits completed BindingRecord formation.
12. BindingRecordValid evaluates pinned historical context, not current authority alone.
13. No BindingRecord exists for an intended movement that never gained governed causal force.
14. Failure to persist BindingRecord does not erase a completed governed mutation.
15. Missing BindingRecord after mutation creates a mandatory binding-closure repair obligation.
16. Reconstructed BindingRecord must carry reconstruction provenance.
17. Historical binding evidence is append-only.
18. Reversal, rollback, revocation, and compensation are later movements, not deletion of history.
19. Cycle closure cannot precede BindingRecord or a lawful non-bind terminal outcome.
20. Continuation cannot bypass unresolved binding uncertainty.
21. BindingRecord must not be agent-authored or agent-issued.
22. Duplicate binding claims must be detected and reconciled.
23. Sovereign engagement does not pre-establish sovereign conclusion.

### Replay, Retry, and Idempotency

Commands are one-use. Idempotency keys are substrate-owned. Retry is lawful only under explicit safe-retry authority or fresh authority. Unknown results must not be silently retried. External idempotency guarantees are evidence, not constitutional proof. Duplicate detection must exist in result, mutation, and audit layers. Prior possible occurrence must be accounted for before retry.
...
Compensation is normally a distinct constitutional movement. Compensation may use fresh Resolution, bounded pre-authorized remedial authority, or sovereign-directed authority. Compensation must not erase historical occurrence. Rollback after binding is a new governed mutation and may form a separate binding event. Rollback attempted but not applied remains evidence only. Compensation failure creates obligation and escalation.

### Audit Durability for F-5
...
Command audit durability is required before attempt. Result audit durability is required before admission. Admission audit durability is required before mutation. Mutation audit durability is required before BindingRecord recording. BindingRecord audit durability is required before CycleRecord and Task Ledger closure. Local append-only durable buffering may satisfy the requirement. Central replication may be delayed under ratified degraded mode. Delayed replication creates an audit-repair obligation. Domain constitutions may impose stricter synchronous requirements. Total durable-capture failure stops externally consequential progression absent explicit sovereign emergency authority.
...
*v5.14: F-3 BindingRecord reconciliation. Section 4 now states that binding occurs at a valid applied GovernedStateMutationRecord and that BindingRecord records and proves binding without creating it. Core owns the canonical BindingRecord schema and BindingRecordValid. The schema includes required task, cycle, Resolution, admission, mutation, authority, domain, bound-scope, binding-type, standing-disposition, timing, recording-mode, provenance, and audit fields, with conditional actuation, standing, unbound-scope, and reconstruction references. StandingDisposition is limited to APPLICABLE_VALID and NOT_APPLICABLE. BindingType is limited to INTENDED_MOVEMENT, OBLIGATION_CREATION, ACCOUNTING, ROLLBACK, COMPENSATION, and NO_OP_GOVERNED_FACT. RecordingMode is limited to CONTEMPORANEOUS and RECONSTRUCTED. The change also defines missing BindingRecord repair handling, CycleRecord Bind outcome lineage, and the 23 formal binding invariants. Constitutional Standing v1.7 supplies standing applicability and historical validation doctrine. Constitutional Task Ledger v1.4 consumes BindingRecord and adds binding-closure repair obligations. CRC-SSR v1.6 resolves `crc-ssr:binding-record` to REGISTERED with Core v5.14 as owner. CRC-OAR v1.2 restarts F-3 by moving OAR-F003 from BLOCKED to SPECIFYING only. F-3 is not SPECIFIED or RESOLVED. F-5 entries are unchanged. Option Y remains in force, so unrelated companion citations are not mass-updated.*

*v5.13: F-5 Substrate Actuation and Effect Admission Phase 1. Section 4 now publishes the Core-owned generic apparatus connecting Resolution(Emit) to governed causal force: ActuationCommand, ActuationResult, ActuationAdmissionRecord, GovernedStateMutationRecord, the substrate-owned actuator interface, actuation command issuance and validity predicates, result validity and admission predicates, governed mutation reachability and validity predicates, audit durability requirements, write-order invariants, replay and retry constraints, and the binding dependency contract needed for later F-3 BindingRecord reconciliation. This phase does not settle BindingRecord, does not restart F-3, does not change CRC-OAR status, and does not register any OAR entry. Constitutional Actuation and Effect Admission v1.0 supplies C10 branch mechanics and conformance doctrine. Constitutional Tools v1.2 and Constitutional Task Ledger v1.3 align their owned surfaces. CRC-SSR v1.5 registers the four new Core-owned shared objects and preserves `crc-ssr:binding-record` as AMBIGUOUS. Option Y remains in force, so unrelated companion citations are not mass-updated.*

*v5.12: F-1 ContinuationState reconciliation. Section 4 now states that ContinuationState is a Core-owned constitutional object and publishes the canonical Core field schema for substrate-issued next-cycle continuation authority. Constitutional Standing remains the owner of ContinuationIssued, ContinuationGrounded, standing-sensitive consumption doctrine, StandingContext, and StandingFailureRecord. Constitutional Task Ledger remains the owner of authoritative task state, CycleRecord, AllowedNextAffordanceSet, pending obligations, and task-status semantics. The canonical ContinuationState schema contains no `derived_from_ref`, no `prior_resolution_ref`, no `state_ref`, no Standing field, no hold, escalation, or reconstitution status field, no BindingRecordRef, no state-mutation field, no effect-result field, and no lifecycle metadata field. Outcome lineage reconstructs through `issuing_resolution_ref`, `task_ledger_ref`, `cycle_record_ref`, and the audit trace. Core defines `ContinuationStateIssuable(c)` as the issuance predicate, while currentness and consumption remain distributed across Standing, Task Ledger, cycle identity, and audit state. This is owner-side Core schema formalization with Standing clarification and CRC-SSR registry reconciliation, not a B.3 cross-paper amendment and not a B.5 compatible extension. F-3 BindingRecord remains unresolved, and admitted-effect lineage and substrate-owned actuation remain outside this reconciliation under provisional F-5. Option Y remains in force, so companion Core-alignment citations are not mass-updated.*
...
The POC's `ContinuationState` dataclass is also illustrative and minimal. It is schema-incomplete relative to the canonical ContinuationState definition in Section 4. The POC field named `prior_resolution_ref` maps conceptually to canonical `issuing_resolution_ref`, and `allowed_next_affordance_set_id` maps conceptually to canonical `allowed_next_affordance_set_ref`. Omitted canonical fields do not create competing schema authority.

The POC's BindingRecord and state-update ordering is illustrative and non-canonical after v5.14. Any apparent BindingRecord-before-state-update sequence in this POC does not override the Section 4 rule that binding occurs at a valid applied GovernedStateMutationRecord and that a completed BindingRecord must never precede the mutation it records.
...
- **Illustrative BindingRecord ordering.** In Cycle 1 the POC forms a `BindingRecord` before `current_state` advances. That ordering is illustrative only and is superseded by v5.14 doctrine: binding occurs at valid applied governed mutation, and BindingRecord records and proves rather than creates binding.
...
### B.12 F-1 ContinuationState Reconciliation

F-1 concerned incomplete Core formalization and schema-location ambiguity for ContinuationState. Core already owned and originated ContinuationState through the ORSR return path: Resolution to ContinuationState to AgentObservation. Core v5.12 now publishes the canonical ContinuationState schema in Section 4.

This is owner-side Core schema formalization with Constitutional Standing clarification and CRC-SSR registry reconciliation. It is not a B.3 cross-paper amendment, because Core is formalizing its own object rather than one paper altering another paper's shared apparatus. It is not a B.5 compatible extension. Constitutional Standing v1.6 clarifies that Standing owns `ContinuationIssued`, `ContinuationGrounded`, standing-sensitive consumption doctrine, StandingContext, and StandingFailureRecord, while Core owns the ContinuationState object schema and issuance predicate.

CRC-SSR v1.4 records the F-1 RESOLVED closure and settles `crc-ssr:continuation-state` to REGISTERED. No new object is created. No consumer migration is required. F-3 BindingRecord remains unresolved, and F-5 admitted-effect lineage and substrate-owned actuation remain outside scope. No new registry activation is performed, and no activation receipt or amendment receipt is required.

### B.13 F-5 Substrate Actuation and Effect Admission Phase 1

F-5 concerned the missing constitutional sequence between adjudicative authorization and governed causal force. Core v5.13 now publishes the generic object apparatus and predicates for ActuationCommand, ActuationResult, ActuationAdmissionRecord, and GovernedStateMutationRecord; the substrate-owned actuator interface; the binding point; and the dependency contract needed by future F-3 BindingRecord reconciliation.

This is coordinated Core apparatus formalization with C10 companion elaboration, Tools v1.2 specialization alignment, Task Ledger v1.3 obligation alignment, and CRC-SSR v1.5 registry update. It is not a B.3 cross-paper amendment because Core is defining its own generic apparatus, C10 is supplying branch mechanics under that apparatus, and the companion revisions align only their owned surfaces. It does not complete F-3, does not settle `crc-ssr:binding-record`, does not change CRC-OAR status, and does not require an activation receipt or amendment receipt.

### B.14 F-3 BindingRecord Reconciliation

F-3 concerned the unresolved BindingRecord lineage across Core, Standing, Task Ledger, F-5 actuation apparatus, and CRC-SSR. Core v5.14 now owns and publishes the canonical BindingRecord schema, the non-constitutive BindingRecord doctrine, BindingRecordValid, the standing-disposition consumption rule, missing-record repair handling, and the CycleRecord Bind lineage requirement.

This is owner-side Core formalization with Standing v1.7 clarification, Task Ledger v1.4 consumer alignment, CRC-SSR v1.6 registry settlement, and CRC-OAR v1.2 lifecycle restart. It is not a B.3 cross-paper amendment because Core is defining its own canonical BindingRecord object and generic predicate, while Standing and Task Ledger align only their owned surfaces. Standing v1.7 contributes standing applicability, non-applicability, failure timing, and historical validation doctrine. Task Ledger v1.4 consumes BindingRecord for CycleRecord Bind and supplies binding-closure repair obligations. CRC-SSR v1.6 resolves the prior `crc-ssr:binding-record` ambiguity. OAR-F003 remains only SPECIFYING after this package; later OAR transition is still required for SPECIFIED and RESOLVED. No activation receipt or amendment receipt is required.
